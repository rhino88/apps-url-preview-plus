      ...
      {isDialogOpen && (
        <Dialog onClose={() => setIsDialogOpen(false)} maxWidth={400}>
          ...
          <Text marginTop={2}>
            ...
            , Figma, SoundCloud, Spotify, Vimeo, YouTube, Loom share links,
            Google Drive share links, Google Docs, Google Sheets, Google Slides
          </Text>
          <Link
            marginTop={2}
...
function RecordPreview({
  ...
  useEffect(() => {
    ...
    const fetchPreviewUrl = async () => {
      // Assuming cellValue is available here, you might need to adapt this
      ...
      const url = await getPreviewUrlForCellValue(cellValue);
      setPreviewUrl(url);
    };
...
async function getPreviewUrlForCellValue(url) {
  if (!url) {
    return null;
  }
  ...
  for (const converter of converters) {
    const previewUrl = await converter(url);
    if (previewUrl) {
...
const converters = [
  ...
  async function getOpenGraphImageUrl(getUrl) {
    ...
    console.log("going?");
    if (!ogImageMetaTag) {
      throw new Error("No og:image meta tag found.");
    }
    const url = ogImageMetaTag.getAttribute("content");
    console.log(url);
